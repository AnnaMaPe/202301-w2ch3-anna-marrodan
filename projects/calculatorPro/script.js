...
const superiorDisplay = document.querySelector(".displaysup");
const inferiorDisplay = document.querySelector(".displayinf");
...
squareRootButton.addEventListener("click", (event) => {
  const value = event.target.innerText;
  const squareRootResult = squareRoot();
  if (!squareRootResult) {
    inferiorDisplay.value = "Syntax ERROR";
    superiorDisplay.value = "➿➿➿➿";
    startOver = true;
...
const result = () => {
  ...
  const checkResult = eval(superiorDisplay.value);
...
const squareRoot = () => {
  const resultSquareRoot = Math.sqrt(inferiorDisplay.value);
