...
const bingoCard = () => {
   ...
   const creatUserBingoCard = () => {
        while (userBingoCard.length < userMaxNumber){
            let cardBingoRandomNumbers = Number((Math.random()*bingoMaxNumber).toFixed())
            const sameCardNumbers = (sameCardNumber) => sameCardNumber === cardBingoRandomNumbers
        ...
        let newCard = confirm('Do you want to keep it?')
        if (!newCard){
            userBingoCard = []
            ...
            alert (`You start with a total of ${initialPunctuation} bingoPoints, for every turn you will be loosing ${turnPunctuation} bingoPoints.`)
...
const generateBingoRandomNumbers = () => {
    let randomNumber = (Math.random()*bingoMaxNumber).toFixed()
    let bingoRandomNumber = Number(randomNumber)
...
const game = () => {
    const sameNumber = () => {
        let numbersToCompare  = userBingoCard.some(e => displayedNumbers.includes(e))
        if (numbersToCompare){
            return true
        } else {
            return false
    ...
    const checkPunctuation = () =>{
        let actualPunctuation = initialPunctuation - (turnPunctuation*displayedNumbers.length)
    ...
    const ranking = () => {
        ...
        players.push({Name: welcomeMessage.charAt(0).toUpperCase() + welcomeMessage.slice(1), Score: userPunctuation})
    ...
    if (!sameNumber()) {
        ...
        let repeatedNumber = Number(displayedNumbers.slice(-1))
        let indexOfRepeatedNumber = userBingoCard.indexOf(repeatedNumber)
        userBingoCard[indexOfRepeatedNumber] = 'X'
        alert("Lucky you! You have that number on your card! Check your updated card down below:")
...
let players = [
    {Name: 'Ted', Score: (initialPunctuation - ((Math.floor(Math.random()*10))*turnPunctuation)) },
    {Name: 'Roy', Score: (initialPunctuation - ((Math.floor(Math.random()*10))*turnPunctuation*2)) },
    {Name: 'Rebecca', Score: (initialPunctuation - ((Math.floor(Math.random()*10))*turnPunctuation)) },
    {Name: 'Keeley', Score: (initialPunctuation - ((Math.floor(Math.random()*10))*turnPunctuation*3))}
]

let welcomeMessage = prompt(`Welcome to LisBingo! What's your name?`)
const letsPlayBingo = () =>{
    if (welcome()) {
        userMaxNumber = +prompt(`What's the maximum amount of numbers you would like to play on your card? It has to be between 5 (shorter game) or 15 (which could be a nightmare if you are unlucky, ma'friend!). Choose wisely`) 
